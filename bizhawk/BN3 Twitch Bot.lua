...
viruslist = require("MMBN 3 Enemies")
AHeld = false
RHeld = false
LHeld = false
BHeld = false
...
function add_chip(c)
	...
	for i = 0,4 do
		...
		if memory.read_u16_le(base_offset + (i*2)) == 218 or memory.read_u16_le(base_offset + (i*2)) == 276 then
			if memory.read_u16_le(0x8011530 + (c*32 - 32) + 0xC) > 0 and memory.read_u16_le(0x8011530 + (c*32 - 32) + 0xC) <= 9999 then
				memory.write_u16_le(0x2034078 + (i*2), memory.read_u16_le(0x2034078 + (i*2)) + memory.read_u16_le(0x8011530 + (c*32 - 32) + 0xC))
				break
			end
		end
		-- Atk+10 and Atk+30
		...
		-- Empty Chip
		if memory.read_u16_le(base_offset + (i*2)) == 0xffff then
			memory.write_u16_le(base_offset + (i*2), c)
			memory.write_u16_le(0x200f862 + (i*2), c)
			memory.write_u16_le(0x203406C + (i*2), memory.read_u16_le(0x8011530 + (c*32 - 32) + 0xC))
			-- Add Damage Boosts
			if memory.read_u16_le(0x8011530 + (c*32 - 32) + 0xC) > 0 and memory.read_u16_le(0x8011530 + (c*32 - 32) + 0xC) <= 9999 then
				memory.write_u16_le(0x2034078 + (i*2), DamageBoosts.Normal)
				DamageBoosts.Normal = 0
				-- Fire
				if memory.readbyte(0x8011530 + (c*32 - 32) + 0x6) == 2 then
					memory.write_u16_le(0x2034078 + (i*2), memory.read_u16_le(0x2034078 + (i*2)) + DamageBoosts.Fire)
					DamageBoosts.Fire = 0
				end
				-- Aqua
				if memory.readbyte(0x8011530 + (c*32 - 32) + 0x6) == 3 then
					memory.write_u16_le(0x2034078 + (i*2), memory.read_u16_le(0x2034078 + (i*2)) + DamageBoosts.Aqua)
					DamageBoosts.Aqua = 0
				end
				-- Wood
				if memory.readbyte(0x8011530 + (c*32 - 32) + 0x6) == 4 then
					memory.write_u16_le(0x2034078 + (i*2), memory.read_u16_le(0x2034078 + (i*2)) + DamageBoosts.Wood)
					DamageBoosts.Wood = 0
				end
				-- Elec
				if memory.readbyte(0x8011530 + (c*32 - 32) + 0x6) == 1 then
					memory.write_u16_le(0x2034078 + (i*2), memory.read_u16_le(0x2034078 + (i*2)) + DamageBoosts.Elec)
					DamageBoosts.Elec = 0
				end
				-- Navi
				if (c >= 219 and c <= 272) or (c >= 277 and c <= 301) then
					memory.write_u16_le(0x2034078 + (i*2), memory.read_u16_le(0x2034078 + (i*2)) + DamageBoosts.Navi)
					DamageBoosts.Navi = 0
				end
			end
			break
...
function sort_chips()
	if battleCheck() then
		...
		for j = 0,5 do
			for i = 0,3 do
				if memory.read_u16_le(base_offset1 + (i*2)) == 0xffff and memory.read_u16_le(base_offset2 + i*2) < 1  and memory.read_u16_le(base_offset3 + i*2) < 1  and memory.read_u16_le(base_offset4 + i*2) < 1 then
					memory.write_u16_le(base_offset1 + (i*2), memory.read_u16_le(base_offset1 + (i*2) + 2))
					memory.write_u16_le(base_offset2 + (i*2), memory.read_u16_le(base_offset2 + (i*2) + 2))
					memory.write_u16_le(base_offset3 + (i*2), memory.read_u16_le(base_offset3 + (i*2) + 2))
					memory.write_u16_le(base_offset4 + (i*2), memory.read_u16_le(base_offset4 + (i*2) + 2))
					memory.write_u16_le(base_offset1 + (i*2) + 2, 0xffff)
					memory.write_u16_le(base_offset2 + (i*2) + 2, 0x0000)
					memory.write_u16_le(base_offset3 + (i*2) + 2, 0x0000)
					memory.write_u16_le(base_offset4 + (i*2) + 2, 0x0000)
				end
			end
		end
		...
		local chipicons = 0x17540
		
		for i = 0,4 do
			for j = 0,0x7f do
				if memory.read_u16_le(base_offset1 + (i*2)) < 0xffff then
					memory.writebyte(chipicons + (i*0x80) + j, memory.readbyte(memory.read_u32_le(chiploc + (memory.read_u16_le(base_offset1 + (i*2))*32) + 20) + j), "VRAM")
				end
			end
		end
	end
	...
	if battleCheck() then
		...
		if TwitchBotVars.Client == nil then
			if memory.readbyte(0x2001889) == 1 then
				...
				if joypad.get().R and not RHeld then
					RHeld = true
					if BHeld then
						chip = chip + 10
					else
				...
				elseif not joypad.get().R then
					RHeld = false
				end
				
				-- L Press Check, Cycle Chip Right
				if joypad.get().L and not LHeld then
					LHeld = true
					if BHeld then
						chip = chip - 10
					else
		...
		if joypad.get().A and not AHeld then
			AHeld = true
			if memory.readbyte(0x203728a) < 5 and memory.readbyte(0x20384d8) > 0 then
				memory.write_u16_le(0x2034060, 0xffff)
				memory.write_u16_le(0x200f862, 0x0000)
				memory.write_u16_le(0x203406C, 0x0000)
				memory.write_u16_le(0x2034078, 0x0000)
...
function changeCurHealth(val)
	...
	if math.abs(tonumber(val)) >= memory.read_u16_le(0x20018A0) and tonumber(val) < 0 then
		memory.write_u16_le(0x20018A0, 0)
		if battleCheck() then
			memory.write_u16_le(0x2037294, memory.read_u16_le(0x20018A0))
			if memory.read_u16_le(0x2037294) >= memory.read_u16_le(0x20018A0) then
				memory.write_u16_le(0x2037294, memory.read_u16_le(0x20018A0))
			end
		end
		fix_hp()
	...
	memory.write_u16_le(0x20018A0, memory.read_u16_le(0x20018A0) + tonumber(val))
	if memory.read_u16_le(0x20018A0) >= memory.read_u16_le(0x20018A2) then
		memory.write_u16_le(0x20018A0, memory.read_u16_le(0x20018A2))
	end
	
	if battleCheck() then
		memory.write_u16_le(0x2037294, memory.read_u16_le(0x20018A0))
		if memory.read_u16_le(0x2037294) >= memory.read_u16_le(0x20018A2) then
			memory.write_u16_le(0x2037294, memory.read_u16_le(0x20018A2))
		end
	end
	fix_hp()
	return val
...
function twitchbot_commands()
	...
	if msg then
		...
		if m and CommandTimers[2] == 0 and (type(user) == "string" or type(mod) == "string") then
			...
			TwitchBotVars.Client:send("PRIVMSG #"..TwitchBotVars.Channel.." :Commands are !difficulty, !chiplist, !stylelist, !fightlist, !slotin, !zennyplz, !fragsplz, !maxhealthplz, !healthplz, !stylechange, !fightme, !scramble, !setstage, !roulettehand, !roulettestage, !chipplz, !queuevirus \r\n")
		end
		...
		-- "Queue Virus" Command
		m, l = string.find(msg:lower(), ":!queuevirus")
		if type(TwitchBotVars.MultiUser) == "string" then
...
function shuffleFolders()
	if not battleCheck() then
		...
		for i = 1, 30, 1 do
			folders.one[i] = memory.read_u32_le(0x02001410 + ((i-1) * 4))
			folders.two[i] = memory.read_u32_le(0x02001410 + ((i-1) * 4) + (60*4))
		end
		...
		for i = 1, 30, 1 do
			memory.write_u32_le(0x02001410 + ((i-1) * 4), folders.one[i])
			memory.write_u32_le(0x02001410 + ((i-1) * 4) + (60*4), folders.two[i])
		end
	end
end
...
function force_encounter(encounter)
	...
	if tonumber(encounter) >= 0 and tonumber(encounter) < 1124 then
		if fights[encounter] > 0 and (memory.read_u32_le(0x2006d0c) == 0 or memory.read_u32_le(0x2006d0c) == 0x200f8c8 or (memory.read_u32_le(0x2006d0c) >= 0x8000000 and memory.read_u32_le(0x2006d0c) < 0x8022000)) then
			memory.write_u16_le(0x2001ddc, 0xFFF0)
			memory.write_u16_le(0x2001de0, 0x0000)
			memory.write_u32_le(0x20018AC, 0x0200f8c8)
			memory.write_u32_le(0x2006d0c, 0x0200f8c8)
			memory.write_u32_le(0x200f8cc, 0x08000000 + fights[encounter])
		end
	elseif tonumber(encounter) == -1 and #QueuedEncounter > 0 then
		-- Cleanup Previous Encounters
		for i=0,6 do
			memory.writebyte(0x2001600+(i*4),255)
			memory.writebyte(0x2001600+(i*4)+1,255)
			memory.writebyte(0x2001600+(i*4)+2,255)
			memory.writebyte(0x2001600+(i*4)+3,255)
		end
		
		-- Write Encounter
		for i=0,#QueuedEncounter+1 do
			if i >= 1 and i <= #QueuedEncounter then
				memory.writebyte(0x2001608+(i*4), QueuedEncounter[i].v_id)
				memory.writebyte(0x2001608+(i*4)+1, QueuedEncounter[i].v_x)
				memory.writebyte(0x2001608+(i*4)+2, QueuedEncounter[i].v_y)
				memory.writebyte(0x2001608+(i*4)+3, 1)
			elseif i == 0 then
				memory.writebyte(0x2001608+(i*4),0)
				memory.writebyte(0x2001608+(i*4)+1,2)
				memory.writebyte(0x2001608+(i*4)+2,2)
				memory.writebyte(0x2001608+(i*4)+3,0)
			else
				memory.writebyte(0x2001608+(i*4),255)
				memory.writebyte(0x2001608+(i*4)+1,0)
				memory.writebyte(0x2001608+(i*4)+2,0)
				memory.writebyte(0x2001608+(i*4)+3,0)
			end
		end
		...
		memory.write_u16_le(0x2001ddc, 0xFFF0)
		memory.write_u16_le(0x2001de0, 0x0000)
		memory.write_u32_le(0x2001600, 0x00002000)
		memory.write_u32_le(0x2001604, 0x02001608)
		memory.write_u32_le(0x20018AC, 0x02001600)
		memory.write_u32_le(0x2006d0c, 0x02001600)
	end
end
...
function rouletteHand()
	if battleCheck() and RouletteHandActive == true then
		...
		local base_offset4 = 0x2034078
		for i = 0,4 do
			memory.write_u16_le(base_offset1 + (i*2), 0xffff)
			memory.write_u16_le(base_offset2 + (i*2), 0x0000)
			memory.write_u16_le(base_offset3 + (i*2), 0x0000)
			memory.write_u16_le(base_offset4 + (i*2), 0x0000)
		end
...
function rouletteStage()
	if battleCheck() and RouletteStageActive == true and RouletteStageTimer % 10 == 0 then
		for i = 0, 23 do
			if not (i==0 or i==7 or i==8 or i==15 or i==16 or i==23) and (memory.readbyte(0x0200F5A0 + (i * 0x10)) == 2 or memory.readbyte(0x0200F5A0 + (i * 0x10)) >= 4) then
...
function fix_hp()
	-- Current Health fix
	if battleCheck() and not (memory.read_u16_le(0x20018A0) == memory.read_u16_le(0x2037294)) then
		memory.write_u16_le(0x20018A0, memory.read_u16_le(0x2037294))
	end
	
	-- Max Health Fix
	if battleCheck() and not (memory.read_u16_le(0x20018A2) == memory.read_u16_le(0x2037296)) then
...
while 1 do
	...
	if battleCheck() then
		...
		if not AllowCustomScreen and memory.readbyte(0x2001886) >= 1 then
			...
			if memory.read_u16_le(0x2006CCC) >= 0x3fe0 then
				memory.write_u16_le(0x2006CCC, 0)
			end
		end
		...
		if not TwitchBotVars.Client then
			gui.text(4*client.getwindowsize(),140*client.getwindowsize(),"Slot-In Chip: "..chiplist.names[chip], "white","black")
		end
	elseif memory.read_u16_le(0x20018a0) > 0 then
		CurrentStyle = tonumber(memory.readbyte(0x2001881))
		if stylelist[CurrentStyle] then
			gui.text(4*client.getwindowsize(),150*client.getwindowsize(),"Style: "..stylelist[CurrentStyle], "white","black")
		end
	end
	...
	if type(ForceNextEncounter) == "number" then
		if ForceNextEncounter >= -1 and ForceNextEncounter < 1124 then
